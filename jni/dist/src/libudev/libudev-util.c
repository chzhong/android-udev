...
#define bufcpy(str,slen,buf,off,len) \
    slen = strlen(str); \
	strncpy(buf + off, str, MAX(slen + 1, len)); \
	str = buf + off; \
	off += slen + 1; \
	len -= slen + 1
...
int getpwnam_r(const char* name, struct passwd* pwd, char* buf, size_t buflen, struct passwd** result) {
    struct passwd * raw = getpwnam(name);
    
    if (raw != NULL) {    
		memcpy(pwd, raw, sizeof(struct passwd));
		
		size_t n = buflen;
		size_t offset = 0;
		size_t slen;
		
		bufcpy(raw->pw_name, slen, buf, offset, n);
		bufcpy(raw->pw_passwd, slen, buf, offset, n);

		bufcpy(raw->pw_dir, slen, buf, offset, n);
		bufcpy(raw->pw_shell, slen, buf, offset, n);
...
int getpwuid_r(uid_t uid, struct passwd *pwd, char *buf, size_t buflen, struct passwd **result) {
    struct passwd * raw = getpwuid(uid);
